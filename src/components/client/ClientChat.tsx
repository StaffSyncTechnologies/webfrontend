...
export const ClientChat: React.FC<ClientChatProps> = ({ clientUserId }) => {
  ...
  // API hooks
  const { data: rooms, isLoading: roomsLoading, refetch: refetchRooms } = useClientGetMyRoomsQuery();
  const { data: unreadCount } = useClientGetUnreadCountQuery();
  const [createRoom] = useClientCreateRoomMutation();
  const { data: messages, isLoading: messagesLoading } = useClientGetRoomMessagesQuery(
    { roomId: selectedRoom?.id || '' },
    { skip: !selectedRoom }
  );
  const [sendMessage] = useClientSendMessageMutation();
  ...
  const handleCreateRoom = async () => {
    try {
      const result = await createRoom().unwrap();
      if (result.success) {
  ...
  const getRoomDisplayName = (room: ChatRoom) => {
    if (room.agencyUser) {
      return room.agencyUser.fullName;
    }
  ...
  const getRoomAvatar = (room: ChatRoom) => {
    if (room.agencyUser) {
      return room.agencyUser.fullName.charAt(0).toUpperCase();
    }
    return 'A';
  };
  ...
  const renderMessage = (message: ChatMessage) => {
    const isOwn = message.senderType === 'client_user';
      ...
      <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
        <Box display="flex" alignItems="center" justifyContent="space-between">
          <Box display="flex" alignItems="center" gap={2}>
            ...
            <Typography variant="h6">Agency Chat</Typography>
            ...
            {roomsData.length > 0 ? (
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary" align="center" sx={{ mb: 3 }}>
                  Start a conversation with our agency support team to get help with your staffing needs.
                </Typography>
                <Button
                  variant="contained"
                  onClick={handleCreateRoom}
