...
export const AdminChatScreen: React.FC<RootStackScreenProps<'AdminChat'>> = ({ navigation, route }) => {
  const insets = useSafeAreaInsets();
  const { primaryColor, secondaryColor } = useOrgTheme();
  const { isDark } = useTheme();
  const scrollRef = useRef<ScrollView>(null);
  const currentUserId = useAppSelector((state) => state?.auth.admin?.id);

  const [inputText, setInputText] = useState('');
  const [roomId, setRoomId] = useState<string | undefined>(route.params?.roomId);
  const [recipientName, setRecipientName] = useState(route.params?.recipientName || 'Unknown');
  ...
  if (!roomId) {
    return (
      <View className="flex-1 bg-light-background-primary dark:bg-dark-background-primary items-center justify-center">
        <Body color="secondary">No chat room selected</Body>
      </View>
    );
  }
  ...
  if (isLoadingMessages) {
    return (
      <View className="flex-1 bg-light-background-primary dark:bg-dark-background-primary items-center justify-center">
        ...
        <Body color="secondary" className="mt-3">Loading chat...</Body>
      </View>
    );
  }
